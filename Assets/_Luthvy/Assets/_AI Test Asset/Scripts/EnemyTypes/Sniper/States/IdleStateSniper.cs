...
/// PROPERTIES OF STATE
    ...
    public void Update()
    {
        ...
        if (enemy.playerInSightRange && enemy.playerInAttackRange)
        {
            ...
            Quaternion lookRotation = Quaternion.LookRotation(shootDir);
            enemy.firePoint.rotation = Quaternion.Slerp(enemy.firePoint.rotation, lookRotation, Time.deltaTime * 5f);

            //if (Physics.Raycast(ray, out hit, enemy.sightRange ,enemy.thePlayer ))
            //{
